...
@allure.feature("Retries")
@allure.story("Concurrency")
async def test_concurrent_mark_for_retry_increments_attempt_count(
    decktask_service,
    db_session: AsyncSession,
    test_user: UserDTO,
    monkeypatch,
    ...
    await asyncio.gather(
        decktask_service.mark_for_retry_or_fail(task.id, RuntimeError("a")),
        decktask_service.mark_for_retry_or_fail(task.id, RuntimeError("b")),
    )
    ...
    assert task.attempt_count == 2
    assert task.status == "RETRY_SCHEDULED"
    assert task.error in {"a", "b"}

@allure.feature("Retries")
@allure.story("Concurrency")
async def test_concurrent_mark_for_retry_fails_exactly_once_after_max_attempts(
    decktask_service,
    db_session: AsyncSession,
    test_user: UserDTO,
    monkeypatch,
    ...
    # ровно один вызов перевёл задачу в FAILED, второй был no-op:
    # attempt_count не задвоился, error записан один раз
    assert task.status == "FAILED"
    assert task.attempt_count == 3

@allure.feature("Retries")
@allure.story("Concurrency")
async def test_concurrent_reschedule_creates_single_outbox_event(
    decktask_service,
    db_session: AsyncSession,
    test_user: UserDTO,
    monkeypatch,
    ...
    results = await asyncio.gather(
        decktask_service.reschedule_for_retry(task.id),
        decktask_service.reschedule_for_retry(task.id),
    )

    # ровно один вызов успешно выполнил claim и создал событие
